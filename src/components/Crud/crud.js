  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          console.log('CRUD refresh - 原始数据:', data)
          ...
          if (data === null || data === undefined) {
            ...
            data = { content: data, totalElements: data.length }
          } else if (!data.content) {
            data = { content: [], totalElements: 0 }
          }
          
          console.log('CRUD refresh - 处理后数据:', data)
          
          crud.page.total = data.totalElements
          crud.data = data.content
          console.log('CRUD refresh - 设置crud.data后:', crud.data)
    ...
    resetDataStatus() {
      ...
      function resetStatus(datas) {
        ...
        datas.forEach(e => {
          if (e && crud.getDataId(e) !== undefined) {
    ...
    getDataStatus(id) {
      if (id === undefined || id === null) {
        return { delete: 0, edit: 0 }
      }
      return crud.dataStatus[id] || { delete: 0, edit: 0 }
    },
    /**
     * 用于树形表格多选, 选中所有
     * @param selection
